...
Cardio sessions accept optional whole-number calories (kcal), average heart rate, distance, average speed, incline, and average indoor-cycling power. Prefer active calories from the machine or watch. **Log metrics / Edit metrics** works for both standalone sessions and cardio linked to a workout. Blank means unknown; zero is a recorded value where valid. The Cardio screen compares calories, fat-energy equivalent, average heart rate, distance, average speed, and average incline across This week, This month, 3 months, 6 months, This year, and All time. Each range except All time is compared with its immediately preceding equivalent period. The week follows the saved week-start preference; month/year are calendar periods to today, and 3/6 months are rolling periods anchored to today. Future sessions are excluded and metric coverage counts make partial data clear.

The minimized Cardio progress card also shows an experimental personal fitness score. Incline treadmill walking uses speed and gradient to estimate workload demand; indoor cycling uses recorded watts. The score compares each activity only with its own initial baseline and excludes missing fields instead of treating them as zero. See [the cardio fitness score design](docs/cardio-fitness-score.md) for the formula, confidence rules, evidence, and limitations.

The comparison uses 7,700 kcal per kg as a conventional approximation and is explicitly not measured fat loss or a food deficit. It does not estimate calories automatically or infer past entries from duration. See [Hall's discussion of the energy rule and its limitations](https://pubmed.ncbi.nlm.nih.gov/17848938/). Run `alembic upgrade head` and restart the backend before serving a metrics-enabled frontend; normal `start-app.ps1` startup handles the migration and build.
...
**Weekly cardio** includes every cardio session. Weekly dashboard totals and goals follow the saved week-start preference and exclude future entries. A Sunday start means Sunday through Saturday.
